...
"""
--------------------------------------------------------------------------------
Created:  Jackson Lee 11/20/14

This script reads in a tab delimited file of sequence headers with md5 keys
and determines a unique search set to lookup in the mgrast md5 tab delimited 
database file. The script then searches the database and outputs a tab delimited 
annotations file for protein/rna, ontology, and organism for each protein 
sequence.  Note that all databases must be in the same directory and conform to 
the original MG-RAST naming format (md5_lca_map, etc.).  Now with cluster file 
support!
...
--------------------------------------------------------------------------------
usage:   query_mgrast_md5_annotation.py -i top.md5.txt -c aa90.cluster.file -e e_val_threshold -d md5_database_directory -o output.prefix -s 1,2,3
"""
#-------------------------------------------------------------------------------
...
if __name__ == '__main__':
    parser = ArgumentParser(usage = "query_mgrast_md5_annotation.py -i top.md5.txt -c aa90.cluster.file -e e_val_threshold -d md5_database_directory -o output.prefix -s 1,2,3",
                            description=__doc__, 
                            formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument("-i", "--input_file", action="store", 
                        dest="inputfilename",
                        help="tab-delimited MGRAST 650 Sims file")
    parser.add_argument("-c", "--cluster_file", action="store", 
                        dest="clusterfilename",
                        help="tab-delimited MGRAST 550 aa90 cluster file")
    parser.add_argument("-d", "--md5_database_directory", action="store", 
                        dest="dbdir",
                        help="tab-delimited MGRAST md5 lca file")                  
    parser.add_argument("-o", "--output_filename", action="store", 
                        dest="outputfilename",
                        help="tab-delimited output file")
    parser.add_argument("-e", "--e_value", action="store", type=float, 
                        dest="ethreshold", default=1.0,
                        help="maximum e-value cutoff (e.g. 1e-10)")              
    parser.add_argument("-t", "--cutoff_id", action="store", type=float, 
                        dest="cutoff", default=1.0,
                        help="minimum id cutoff in percent (e.g. 50)")
    parser.add_argument("-s", "--source_filters", action="store", dest="sources", 
                        default="all", help="comma-delimited list of source \
type restrictions, prioritized first e.g. 11,12,13, use \"view\" to see list")                  
