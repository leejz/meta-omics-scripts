...
"""
--------------------------------------------------------------------------------
Created:  Jackson Lee 11/21/14
This script reads in a counts, phylogeny, bin, function, ontology and taxonomy 
tab-delimited files and combines identical header records in a tab delimited 
format.  This script will take counts from each sample and normalize all genes 
to the sample average per base RPKM.  This combined data is written as a 
tab-delimited file for each unique sequence file header.
   
Then the script sums duplicate ontology-organism RPKM coverages and consolidates
the records, sorts by bin then ontology, and writes an excel tab delimited file
with RPKMs for each sample.
...
Output file format:
database format:
header	bin_num	function	ontology	organism	taxonomy	Sample_name_RPKM	etc.	bplen
...
--------------------------------------------------------------------------------   
usage:   combine_counts_bins_func_ont_tax.py -c counts.txt -b bins.txt -f function.txt -y ontology.txt -t taxa.txt -o out.prefix
"""
#-------------------------------------------------------------------------------
...
if __name__ == '__main__':
    parser = ArgumentParser(usage = "combine_counts_bins_func_ont_tax.py -c \
counts.txt -b bins.txt -f function.txt -y ontology.txt -t taxa.txt -o out.prefix",
                            description=__doc__, 
                            formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument("-c", "--counts_file", action="store", 
                        dest="coveragefilename",
                        help="tab delimited index file of all count libraries")
    parser.add_argument("-b", "--bin_file", action="store", 
                        dest="binfilename",
                        help="tab delimited bin file")
    parser.add_argument("-f", "--function_file", action="store", 
                        dest="functionfilename",
                        help="tab delimited protein function file")
    parser.add_argument("-y", "--ontology_file", action="store", 
                        dest="ontologyfilename",
                        help="tab delimited ontology file (See DocString for format)")
    parser.add_argument("-t", "--taxonomy_file", action="store", 
                        dest="taxonomyfilename",
    ...
    for line in coverage_data:
        ...
        if faaname in annotations_list[3]:
            ...
            tax = ''
            org = ''
        output_list.append([faaname, bin, function, ontology, org, tax] + coverages + [bplen])
    ...
    with open(outputdatafilename, 'w') as outputdatafile:
        ...
        writer.writerow(['header', 'bin_num', 'function', 'ontology', 'organism', 'taxonomy'] + sample_names + ['ORF length'])
