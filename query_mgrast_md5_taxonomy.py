...
"""
--------------------------------------------------------------------------------
Created:  Jackson Lee 11/11/14

This script reads in a tab delimited file of mg-rast organism annotations with 
md5 keys and determines a unique search set to lookup in the mgrast md5 tab 
delimited database file. The script then searches the database and outputs a tab 
delimited taxonomic lineage file.
...
--------------------------------------------------------------------------------
usage:   query_mgrast_md5_taxonomy.py -i mgrast_organism.txt -t md5_lca_map.tab -o output.file
"""
#-------------------------------------------------------------------------------
...
if __name__ == '__main__':
    parser = ArgumentParser(usage = "query_mgrast_md5_taxonomy.py -i \
mgrast_organism.txt -t md5_lca_map.tab -o output.file",
                            description=__doc__, 
                            formatter_class=RawDescriptionHelpFormatter)
    parser.add_argument("-i", "--input_file", action="store", 
                        dest="inputfilename",
                        help="tab-delimited MGRAST organism annotation file")
    parser.add_argument("-t", "--md5_lca_file", action="store", 
                        dest="taxfilename",
